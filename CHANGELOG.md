...
- [x] 刘海字幕声波层（覆盖所有说话场景）
- [x] #20 主动消息 TTS 朗读开关（默认关，待实测）
- [x] #21 预制台词管理（编辑/新增/删除/重置 + 闲聊池 + 冷却 + TTS + 音频管理，端到端生效，待实测）
- [x] #22 一起看插件（Ctrl+Shift+S 触发；自动截屏 + LLM 视觉分析 + 气泡/表情/TTS；Esc 退出；设置页可配间隔/提示词/视觉检测，待实测）
