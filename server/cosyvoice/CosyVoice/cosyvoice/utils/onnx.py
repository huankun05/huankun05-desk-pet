...
if onnx_path is not None:
    cuda_providers = onnxruntime.get_available_providers()
    if 'CUDAExecutionProvider' in cuda_providers:
        embedding_extractor, online_feature = EmbeddingExtractor(model_path=os.path.join(onnx_path, 'campplus.onnx')), True
    else:
        print(f"CUDAExecutionProvider not available (got: {cuda_providers}), using CPU fallback for online feature extraction")
        embedding_extractor, online_feature = EmbeddingExtractor(model_path=os.path.join(onnx_path, 'campplus.onnx')), True
else:
