...
class CosyVoiceModel:
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            ...
            while True:
                # 10ms 轮询（原 100ms），显著降低流式首字延迟，CPU 开销可忽略
                time.sleep(0.01)
        ...
        # NOTE: removed torch.cuda.empty_cache() + synchronize() to avoid
        #       redundant GPU memory reallocation on every inference call.
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            token_offset = 0
            _stream_chunk_count = 0  # bistream mel 累积控制：定期重置 hift cache
            prompt_token_pad = int(np.ceil(flow_prompt_speech_token.shape[1] / self.token_hop_len) * self.token_hop_len - flow_prompt_speech_token.shape[1])
            while True:
                time.sleep(0.01)  # 10ms 轮询，降低 chunk 产出延迟
                ...
                if len(self.tts_speech_token_dict[this_uuid]) - token_offset >= this_token_hop_len + self.flow.pre_lookahead_len:
                    ...
                    yield {'tts_speech': this_tts_speech.cpu()}
                    # bistream 优化：定期重置 hift cache，避免 mel 累积导致后段变慢
                    # CosyVoice3Model.token2wav 会 concat 历史 mel → hift O(n²) 变慢
                    # CausalHiFTGenerator 不依赖外部 cache_source，重置安全
                    # 每 4 个 chunk（~4s 音频）重置一次，平衡音质和性能
                    _stream_chunk_count += 1
                    if _stream_chunk_count % 4 == 0:
                        self.hift_cache_dict[this_uuid] = None
        ...
        # NOTE: removed torch.cuda.empty_cache() + synchronize() to avoid
        #       redundant GPU memory reallocation on every inference call.
        #       This significantly reduces RTF (from ~6 to ~1-2 on RTX 4070).
