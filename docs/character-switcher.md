...
3. **`src/renderer/settings/character-style/dom.ts`**（新建）
   - 角色与风格面板所有 DOM 元素引用

4. **`src/renderer/settings/character-style/panel.ts`**（新建）
   - 角色与风格面板完整逻辑：角色列表渲染、编辑/新建/删除、自定义风格跳转
   - `initCharacterStylePanel()` 初始化函数
...
| 聊天界面标题 | 从"回复风格"改为"角色切换" |
| 自定义跳转 | 点击"自定义风格"跳转到设置 → 偏好设置面板 |
| 角色与风格设置项 | 设置窗口左侧导航新增"角色与风格"独立面板 |
...
1. 打开设置窗口
2. 左侧导航选择「角色与风格」
3. 角色列表显示所有角色，每个角色显示名称、风格、模型路径
4. **编辑角色**：点击角色旁的「编辑」按钮，修改名称、模型路径、绑定风格，保存
5. **新建角色**：点击「+ 新建角色」按钮，填写名称、模型路径、选择风格，保存
6. **删除角色**：点击角色旁的「删除」按钮，确认后删除（默认角色昔涟不可删除）
7. 面板底部「自定义风格」区可跳转到偏好设置配置自定义风格参数
