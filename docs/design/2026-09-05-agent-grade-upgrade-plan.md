...
| 2026-09-05 | ④Git Worktree 隔离实施（实现完成，测试待明日收尾） | 移植 Hermes `-w` 工作区隔离：新增 worktree.ts（setupWorktree/cleanupWorktree/worktreeHasUnpushedCommits/detectGitRepoRoot + .worktreeinclude 复制白名单 + 目录穿越拒绝 + 无远端基线视为无未推送，与 Hermes 语义逐条对齐）；harness-adapter 接入创建/清理，cyrene-agent.ts 新增 useWorktree 选项（默认 false）；worktree.test.ts 13 用例编写完成，其中"有未推送提交时保留 worktree"用例按 Hermes 同法修正（配置 remote + update-ref 伪造远端跟踪 ref）；**待明日运行 vitest 验证 + 全量回归 + tsc + 提交** |
| 2026-09-06 | ④Git Worktree 隔离验证完成 | worktree.test.ts 12 用例全部通过；修复 Windows 8.3 短路径问题（os.tmpdir 返回 SHANGM~1 短名）：detectGitRepoRoot/setupWorktree 用 fs.realpathSync.native 展开长路径（避免 path.relative 误判目录逃逸），测试 createRepo 同步归一化；git branch 分支名断言修正（worktree 检出分支带 `+` 前缀、分支名为完整 `cyrene/<name>`）；清理误生成的 clearInterval(i) 空文件与 .cyrene-temp/（加 .gitignore）；主进程 tsc 通过；全量回归 3483/3484 通过（唯一失败为环境缺 Git Bash 的既有问题）；同时修复 3 个既有过时断言（agui-bridge think 流式拆分、skills 独立导航、pearl-white 默认主题） |
| 2026-09-06 | LSP 进程 shell 解析错乱修复 | 排查全量测试生成的 clearInterval(i) 垃圾文件，定位 createLSPProcess 在 Windows 无条件 shell:true：cmd.exe 把 `node -e` 内联脚本中的括号拆解成多条命令并生成同名空文件（复现确认）；修复为 shell 仅对 `.cmd/.bat` 启用，普通命令直接 spawn 原样传参（同时消除 Node DEP0190 注入警告）；lsp-process 14 单测通过 |
| 2026-09-06 | ⑤文件系统快照与 /rollback 回滚交互实施完成 | 新增 checkpoint-manager.ts（基于 Git 的透明文件系统快照：共享对象库 + 每轮去重 + 快照列表/diff/单文件回滚 + pre-rollback 可撤销撤销 + 排除规则/大小上限/数量修剪）；harness 每轮 newTurn()、文件修改类工具（risk=fs-write/shell）dispatch 前自动 ensureCheckpoint（LLM 不可见）；harness-adapter 捕获原始工作区根，worktree 隔离时快照面向用户主项目；agui-bridge 拦截 /rollback 命令（list/diff/<target> [file]）直接响应；21 单测 + 受影响的 tool-dispatcher/harness/harness-adapter/agui-bridge 97 测试全部通过 + tsc 通过 |
