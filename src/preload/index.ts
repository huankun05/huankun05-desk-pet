...
const settingsApi = {
  ...
  hermes: {
    getSettings: () => ipcRenderer.invoke(IPC.HERMES_GET_SETTINGS),
    saveSettings: (patch: unknown) => ipcRenderer.invoke(IPC.HERMES_SAVE_SETTINGS, patch),
    generateKey: () => ipcRenderer.invoke(IPC.HERMES_GENERATE_KEY),
  ...
  storage: {
    ...
    clean: (keys?: string[]) => ipcRenderer.invoke(IPC.STORAGE_CLEAN, keys),
    openPath: (target: string) => ipcRenderer.invoke(IPC.STORAGE_OPEN_PATH, target),
    setLocation: (target: string | null) => ipcRenderer.invoke(IPC.STORAGE_SET_USER_DATA_DIR, target),
    getLocation: () => ipcRenderer.invoke(IPC.STORAGE_GET_LOCATION),
    listBackups: () => ipcRenderer.invoke(IPC.STORAGE_LIST_BACKUPS),
    createBackup: (category?: string) => ipcRenderer.invoke(IPC.STORAGE_CREATE_BACKUP, category),
  },
  ...
  dataUsageScan: () => ipcRenderer.invoke(IPC.DATA_USAGE_SCAN),
  dataUsageClean: () => ipcRenderer.invoke(IPC.DATA_USAGE_CLEAN),
  dataUsagePaths: () => ipcRenderer.invoke(IPC.DATA_USAGE_PATHS),
  dataUsageOpenPath: (target: string) => ipcRenderer.invoke(IPC.DATA_USAGE_OPEN_PATH, target),
