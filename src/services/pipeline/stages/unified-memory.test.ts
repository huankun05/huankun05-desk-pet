...
function mkCtx(userText = '今天天气真好') {
  return {
    userText,
    session: {} as any,
...
describe('UnifiedMemoryStage 路由（统一本地单方案）', () => {
  it('RAG 开启：始终走本地 RAG 检索（不再区分后端在线/离线）', async () => {
    ...
    mockedGetRAGEngine.mockReturnValue(ragStub as any);

    const stage = new UnifiedMemoryStage(() => LOCAL);
    const ctx = mkCtx();
    await stage.process(ctx, {} as any);
  ...
  it('本地 RAG 检索异常时优雅降级（不抛出，跳过长期记忆）', async () => {
    ...
    const stage = new UnifiedMemoryStage(() => LOCAL);
    const ctx = mkCtx();
    await expect(stage.process(ctx, {} as any)).resolves.toBeUndefined();
    expect(ctx.memoryContext).toBe(LOCAL);
  });
});
