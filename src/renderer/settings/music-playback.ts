...
export async function requestTrackPlayback(
  ...
  if (!result.ok) {
    return { kind: "err", message: tOr("music.playback.requestFailed", "播放请求失败：") + (result.errorCode ?? "E_UNKNOWN") };
  }
  if (result.data?.state === "dispatched") {
    return { kind: "ok", message: tOr("music.playback.started", "已开始播放：") + track.name };
  }
  if (result.data?.state === "client_unavailable") {
    return {
      kind: "err",
      message: tOr("music.playback.mpvNotFoundPrefix", "已找到《") + track.name + tOr("music.playback.mpvNotFoundSuffix", "》，但 mpv 播放器未就绪。请重启应用，或确认 resources/bin/mpv/mpv.exe 存在。"),
    };
  }
  if (result.data?.state === "launch_failed") {
    return { kind: "err", message: tOr("music.playback.mpvLaunchFailedPrefix", "mpv 启动失败，无法播放《") + track.name + tOr("music.playback.mpvLaunchFailedSuffix", "》。") };
  }
  if (result.data?.state === "web_fallback") {
    // 兼容旧 IPC 返回值；新架构不应到达此分支。
    return { kind: "ok", message: tOr("music.playback.openedInBrowser", "已在浏览器中打开：") + track.name };
  }
  return { kind: "err", message: tOr("music.playback.playFailedPrefix", "未能播放《") + track.name + tOr("music.playback.playFailedSuffix", "》：") + (result.data?.state ?? "unknown") };
}
