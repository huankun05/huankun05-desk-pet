...
import type { VisionProviderConfig, ProviderMeta } from '../../../services/provider/types';
...
type FormShape = Omit<VisionProviderConfig, 'id' | 'type'>;

const defaultForm: FormShape = {
  name: '',
  enable: true,
  typeName: 'openai_vision',
  apiKey: '',
...
const DEFAULT_ENDPOINTS: Record<string, string> = {
  openai_vision: 'https://api.openai.com/v1',
};
...
export function VisionPage() {
  const [configs, setConfigs] = useState<VisionProviderConfig[]>(
    () => providerManager.listProviders('vision') as VisionProviderConfig[],
  );
  const [activeId, setActiveId] = useState<string | null>(() => {
    const active = providerManager.getActiveVisionProvider();
    return active ? active.config.id : null;
  });
  const [showModal, setShowModal] = useState(false);
  const [editingConfig, setEditingConfig] = useState<VisionProviderConfig | null>(null);
  const [testResults, setTestResults] = useState<Record<string, TestResult>>({});
  const [testingId, setTestingId] = useState<string | null>(null);
  const [visionTypes] = useState<ProviderMeta[]>(() =>
    providerRegistry.getRegisteredTypes('vision'),
  );
  const { t } = useTranslation();
  const { showToast } = useToast();
  const { confirm } = useConfirm();

  const visionEngines: SetupEngineInfo[] = visionTypes.map((m) => ({
    typeName: m.typeName,
  ...
  const loadConfigs = () => {
    const all = providerManager.listProviders('vision') as VisionProviderConfig[];
    setConfigs(all);
    const active = providerManager.getActiveVisionProvider();
  ...
  const { handlePointerDown, draggingId, dragPos, dragSize, visualItems, containerRef } =
    useReorder(configs, {
      onReorder: (orderedIds) => {
        providerManager.reorderProviders('vision', orderedIds);
        loadConfigs();
      },
  ...
  const openEditModal = (config: VisionProviderConfig) => {
  ...
  const wizardSave = (cfg: FormShape, editingId: string | null): boolean => {
    if (editingId) {
      providerManager.updateProvider(editingId, cfg as Partial<VisionProviderConfig>);
    } else {
      const newConfig: VisionProviderConfig = {
        ...cfg,
        id: `vision-${crypto.randomUUID()}`,
        type: 'vision',
      } as VisionProviderConfig;
  ...
  const handleSetActive = (id: string) => {
    ...
    providerManager.setActiveVisionProvider(id);
    setActiveId(id);
  ...
  const runValidation = async (
    ...
    try {
      ...
      const testConfig: VisionProviderConfig = { ...config, id: tempId, enable: true };
      const provider = providerRegistry.createVisionProvider(config.typeName, testConfig);
  ...
  const getTypeNameDisplay = (typeName: string) => {
    const meta = visionTypes.find((x) => x.typeName === typeName);
    return meta?.displayName || typeName;
  };

  const renderCardContent = (config: VisionProviderConfig) => (
  ...
  return (
    <div className="p-4 animate-[fade-in-up_0.3s_ease-out]">
      <Section
        title={t('settings.services.vision_section_title')}
        description={t('settings.services.vision_section_desc')}
      >
        <div className="p-4">
      ...
      <ServiceWizard<FormShape>
        ...
        validate={(cfg) => validateProviderConfig('vision', cfg as VisionProviderConfig)}
        save={wizardSave}
      ...
      <Section title={t('settings.related_settings')}>
        <div className="space-y-2 p-4">
          <SettingsJumpButton
            ...
            label={t('settings.services_section.multimodal')}
            icon="solar:camera-bold-duotone"
            hint={t('settings.services_section.related_vision_hint')}
          />
          <SettingsJumpButton
