...
export class LAppView {
  ...
  /**
   * CSS ピクセル座標から直接焦点を設定する。
   * TouchManager を経由せず、補間座標を直接渡す。
   * @param cssX — CSS ピクセル X（0 〜 canvas.clientWidth）
   * @param cssY — CSS ピクセル Y（0 〜 canvas.clientHeight）
   */
  public setFocusFromCss(cssX: number, cssY: number): void {
    const deviceX = cssX * window.devicePixelRatio;
    const deviceY = cssY * window.devicePixelRatio;

    // === 偏移调节区域 ===
    // offsetX: X 轴偏移（正值向右，负值向左）
    // offsetY: Y 轴偏移（正值向下，负值向上）
    const offsetX = 0; // 在这里调节 X 轴偏移（像素）
    const offsetY = 0; // 在这里调节 Y 轴偏移（像素）

    const viewX = this.transformViewX(deviceX + offsetX);
    const viewY = this.transformViewY(deviceY + offsetY);
