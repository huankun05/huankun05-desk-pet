...
import { collectAllPresetTexts, syncInteractionMessagesFromBackend } from './data/idleMessages';
...
// 先同步后端台词（Core:9877 为文本权威源），确保预热用的是后端文本而非默认/旧文本。
void (async () => {
  await syncInteractionMessagesFromBackend();
  await interactTTS.prewarm(collectAllPresetTexts());
})().catch(() => undefined);
